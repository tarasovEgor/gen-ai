...
_VALID_METRICS = frozenset(
    {"key_rate", "fx_USD", "fx_EUR", "fx_CNY", "cpi", "unemployment"}
)
...
def _metric_value(metric: str, period: str) -> dict:
    """Получить одно значение метрики на период через существующие инструменты."""
    iso_date, year, month = _parse_period(period)
    if metric == "key_rate":
        return get_key_rate(on_date=iso_date)
    if metric.startswith("fx_"):
        currency = metric.split("_", 1)[1]
        return get_fx_rate(currency=currency, on_date=iso_date)
    if metric == "cpi":
        return get_inflation(year=year, month=month)
    if metric == "unemployment":
        return get_unemployment(year=year, month=month)
    return {"error": f"неизвестная метрика: {metric}"}


def _extract_numeric(obs: dict, metric: str) -> tuple[float | None, str | None, str | None]:
    """Достать число, дату и source из ответа инструмента."""
    if "error" in obs:
        return None, None, None
    if metric == "key_rate":
        return float(obs["rate"]), obs.get("date"), obs.get("source")
    if metric.startswith("fx_"):
        return float(obs["rate"]), obs.get("date"), obs.get("source")
    if metric == "cpi":
        d = f"{obs['year']}-{obs['month']:02d}"
        return float(obs["cpi_yoy"]), d, obs.get("source")
    if metric == "unemployment":
        d = f"{obs['year']}-{obs['month']:02d}"
        return float(obs["unemployment"]), d, obs.get("source")
    return None, None, None
...
def compare_periods(
    ...
    Использует get_fx_rate, get_key_rate, get_inflation, get_unemployment.
    """
    metric = metric.strip()
    if metric not in _VALID_METRICS:
        return {
            "error": f"metric должен быть одним из {sorted(_VALID_METRICS)}, получено {metric!r}"
        }

    raw_a = _metric_value(metric, period_a)
    raw_b = _metric_value(metric, period_b)
    val_a, date_a, src_a = _extract_numeric(raw_a, metric)
    val_b, date_b, src_b = _extract_numeric(raw_b, metric)

    if val_a is None:
        return {"error": f"период A ({period_a}): {raw_a.get('error', raw_a)}"}
    if val_b is None:
        return {"error": f"период B ({period_b}): {raw_b.get('error', raw_b)}"}
    if val_a == 0:
        return {"error": "значение в периоде A равно нулю, ratio не определён"}

    sources = sorted({s for s in (src_a, src_b) if s})
